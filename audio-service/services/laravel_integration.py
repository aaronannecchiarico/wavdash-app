...
class CallbackDeliveryService:
    ...
    def send_callback(self, callback_url: str, callback_data: EnhancedCallbackData, 
                     priority: CallbackPriority = CallbackPriority.NORMAL) -> bool:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code in [200, 201, 202]:
                    ...
                    if response.text:
                        try:
                            ...
                            logger.debug(f"Laravel response (text): {response.text[:200]}")
                    
                    return True
                
                else:
                    error_msg = f"HTTP {response.status_code}: {response.text[:200]}"
                    logger.warning(f"Callback failed with status {response.status_code}: {error_msg}")
                ...
                logger.warning(f"Callback timeout (attempt {attempt + 1}) to {callback_url}")
            
            except requests.exceptions.ConnectionError:
                logger.warning(f"Connection error (attempt {attempt + 1}) to {callback_url}")
            
            except requests.exceptions.RequestException as e:
                logger.warning(f"Request error (attempt {attempt + 1}) to {callback_url}: {str(e)}")
            
            except Exception as e:
                logger.error(f"Unexpected error (attempt {attempt + 1}) sending callback: {str(e)}")
        ...
        # All attempts failed
        logger.error(f"❌ Failed to deliver callback to {callback_url} after {max_retries + 1} attempts")
    ...
    def _update_response_time_stats(self, response_time: float):
        ...
        new_avg = ((current_avg * (total_attempts - 1)) + response_time) / total_attempts
...
class LaravelIntegrationHelper:
    ...
    def create_tempo_callback_data(
        ...
        if status == CallbackStatus.COMPLETED and processing_result:
            ...
            callback_data.processing_warnings = processing_result.get("processing_warnings")
            ...
            callback_data.error_code = processing_result.get("error_code", "PROCESSING_FAILED")
    ...
    def validate_callback_url(callback_url: str) -> bool:
        ...
        try:
            ...
            if response.status_code >= 500:
                logger.warning(f"Callback URL returned server error: {response.status_code}")
                return False
