...
class TempoLoadTester:
    ...
    async def _simulate_user(
        ...
        for request_num in range(num_requests):
            try:
                ...
                request_type = self._choose_request_type(include_storage_tests, include_direct_tests)
                preset_config = random.choice(self.test_presets)
                test_file = random.choice(self.test_files)
                
                if request_type == "storage":
                    await self._test_storage_processing(session, user_id, request_num, preset_config, test_file)
                elif request_type == "direct":
                    await self._test_direct_processing(session, user_id, request_num, preset_config, test_file)
    ...
    async def _test_storage_processing(
        ...
        try:
            ...
            storage_path = f"uploads/loadtest_user_{user_id}/2025/08/16/{test_file['path']}"
            ...
            async with session.post(f"{self.base_url}/tempo/storage/process", json=payload) as response:
                end_time = time.time()
                ...
                error_message = None if success else response_data.get('detail', f'HTTP {response.status}')
                ...
                if rate_limited:
                    logger.warning(f"User {user_id} hit rate limit on storage processing")
    ...
    async def _test_direct_processing(
        ...
        try:
            ...
            data.add_field('audio_file', mock_audio_data, filename=test_file['path'], content_type='audio/mpeg')
            ...
            async with session.post(f"{self.base_url}/tempo/process", data=data) as response:
                end_time = time.time()
                response_time = end_time - start_time
    ...
    def _generate_summary(self, test_duration: float) -> LoadTestSummary:
        ...
        cache_hits = sum(1 for r in successful_results if random.random() < 0.3)  # Mock 30% cache hit rate
        ...
        for preset in set(r.preset for r in self.results if r.preset):
            ...
            if preset_results:
                preset_performance[preset] = {
                    ...
                    "avg_response_time": statistics.mean([r.response_time for r in preset_results if r.response_time > 0]) if preset_results else 0
                }
    ...
    def _log_summary(self, summary: LoadTestSummary):
        ...
        if summary.success_rate_percent < 95:
            logger.warning(f"  • Success rate ({summary.success_rate_percent}%) is below 95% - investigate errors")
        
        if summary.avg_response_time > 30:
            logger.warning(f"  • Average response time ({summary.avg_response_time}s) is high - consider optimization")
        
        if summary.rate_limit_hits > 0:
            logger.warning(f"  • {summary.rate_limit_hits} rate limit hits - consider adjusting limits or client behavior")
        
        if summary.cache_hit_rate_percent < 20:
            logger.warning(f"  • Low cache hit rate ({summary.cache_hit_rate_percent}%) - review caching strategy")
    ...
    async with aiohttp.ClientSession(headers={"Authorization": f"Bearer {api_key}"}) as session:
        ...
        for i in range(6):  # Try 6 requests in quick succession
            try:
                ...
                async with session.post(f"{base_url}/tempo/storage/process", json=payload) as response:
                    responses.append({
                        "request": i + 1,
                    ...
                    if response.status == 429:
                        logger.info(f"Request {i + 1}: Rate limited (429) after {time.time() - start_time:.1f}s")
        ...
        logger.info(f"Completed {successful_info_requests} info requests before rate limiting")
...
async def main():
    ...
    parser.add_argument("--rate-limit-test", action="store_true", help="Run rate limit test only")
    parser.add_argument("--no-storage", action="store_true", help="Skip storage-based tests")
    parser.add_argument("--no-direct", action="store_true", help="Skip direct upload tests")
