...
def test_feature_extraction():
    ...
    try:
        ...
        if 'metadata' in result:
            ...
            print(f"   Processed in Chunks: {metadata.get('processed_in_chunks', False)}")
        ...
        if 'features' in result:
            ...
            if 'energy' in features:
                ...
                print(f"   Loudness: {energy_info.get('overall_loudness_db', 0):.1f} dB")
                print(f"   Dynamic Range: {energy_info.get('dynamic_range_db', 0):.1f} dB")
        ...
        if 'chunks' in result_chunked:
            ...
            if 'aggregated_features' in result_chunked:
                ...
                if 'tempo' in agg:
                    print(f"   Average BPM across chunks: {agg['tempo'].get('mean_bpm', 0):.1f}")
                if 'key' in agg:
                    print(f"   Most likely key: {agg['key'].get('most_likely_key', 'unknown')}")
                    print(f"   Key changes detected: {agg['key'].get('key_changes', 0)}")
