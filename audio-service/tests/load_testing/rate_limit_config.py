...
def create_rate_limit_response(
    exception: RateLimitExceeded, limit_key: str, request: Request
) -> JSONResponse:
    ...
    message = error_messages.get(
        category, "Rate limit exceeded. Please wait before making more requests."
    )
...
class AdaptiveRateLimiter:
    ...
    def adjust_limits_based_on_load(self, system_metrics: Dict[str, Any]):
        ...
        load_score = (
            cpu_usage * 0.3
            + memory_usage * 0.2
            + min(avg_response_time / 30 * 50, 50)
            * 0.3  # Cap response time impact at 30s
            + error_rate * 0.1
        ...
        if load_score > 80:
            ...
            new_multiplier = 0.5
            logger.warning(
                f"High system load ({load_score:.1f}), reducing rate limits by 50%"
            )
        elif load_score > 60:
            # Medium load - slightly reduce limits
            new_multiplier = 0.75
            logger.info(
                f"Medium system load ({load_score:.1f}), reducing rate limits by 25%"
            )
        elif load_score < 30:
            # Low load - can increase limits
            new_multiplier = 1.5
            logger.info(
                f"Low system load ({load_score:.1f}), increasing rate limits by 50%"
            )
        else:
        ...
        if abs(new_multiplier - self.current_multiplier) > 0.1:
            ...
            logger.info(
                f"Adjusted rate limit multiplier to {self.current_multiplier:.2f}"
            )
...
class RateLimitMiddleware:
    ...
    async def _track_request(self, request: Request):
        ...
        if self.request_counts[key] > 100:  # More than 100 requests in 5 minutes
            logger.warning(
                f"High request rate from {client_ip} to {endpoint}: {self.request_counts[key]} requests"
            )
...
def get_rate_limit_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        stats["top_limited_ips"] = sorted(
            stats["active_limits"].items(), key=lambda x: x[1], reverse=True
        )[:10]
