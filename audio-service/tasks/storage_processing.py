...
@celery_app.task(bind=True, name='tasks.storage_processing.process_audio_features_from_storage', queue='audio_features')
def process_audio_features_from_storage(
    ...
    try:
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 0, 'status': 'Initializing storage connection', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 30, 'status': 'Extracting audio features', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 80, 'status': 'Uploading results to storage', 'task_id': task_id}
        )
        ...
        storage_analysis_path = storage.upload_analysis_result(feature_result, base_path, analysis_type, user_id)
        ...
        if 'features' in feature_result:
            ...
            logger.debug(f"Using aggregated features from {feature_result.get('chunk_count', 0)} chunks")
        
        if features:
            # Handle different data structures for single vs chunked processing
            bpm = (features.get('tempo', {}).get('bpm', 0) or 
                   features.get('tempo', {}).get('mean_bpm', 0))
            
            key = (features.get('key', {}).get('key') or 
                   features.get('key', {}).get('most_likely_key', 'unknown'))
            
            key_confidence = features.get('key', {}).get('confidence', 0)
            
            loudness_db = (features.get('energy', {}).get('overall_loudness_db', 0) or 
                          features.get('energy', {}).get('mean_loudness_db', 0))
            
            brightness = (features.get('spectral', {}).get('spectral_centroid', {}).get('mean', 0) or 
                         features.get('spectral', {}).get('mean_brightness', 0))
            ...
            musical_analysis = {
                ...
                'duration': float(feature_result.get('metadata', {}).get('duration', 0))
            }
            logger.info(f"Generated musical analysis: BPM={musical_analysis['bpm']:.1f}, Key={musical_analysis['key']}, Duration={musical_analysis['duration']:.1f}s")
        else:
            logger.warning(f"No 'features' or 'aggregated_features' key in feature_result. Available keys: {list(feature_result.keys())}")
        ...
        if callback_url:
            try:
                callback_data = StorageCallbackData(
                    ...
                    storage_paths={"analysis": storage_analysis_path, "original": storage_path},
                    analysis_summary=musical_analysis,
                    processing_time=processing_time,
                    storage_type=storage_type.value
                )
                ...
                logger.error(f"Failed to send callback for task {task_id}: {callback_error}")
        ...
        if callback_url:
            try:
                ...
                logger.error(f"Failed to send failure callback for task {task_id}: {callback_error}")
...
@celery_app.task(bind=True, name='tasks.storage_processing.separate_audio_stems_from_storage', queue='stem_separation')
def separate_audio_stems_from_storage(
    ...
    try:
        ...
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 20, 'status': 'Loading audio and model', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 30, 'status': 'Separating audio stems', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 70, 'status': 'Saving separated stems', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS', 
            meta={'progress': 85, 'status': 'Uploading stems to storage', 'task_id': task_id}
        )
        ...
        if callback_url:
            try:
                ...
                logger.info(f"Callback sent for stem task {task_id}: {response.status_code}")
                
            except Exception as callback_error:
                logger.error(f"Failed to send callback for stem task {task_id}: {callback_error}")
        ...
        if callback_url:
            try:
                ...
                logger.error(f"Failed to send failure callback for stem task {task_id}: {callback_error}")
        ...
        if temp_input_path and os.path.exists(temp_input_path):
            try:
                ...
                logger.warning(f"Failed to cleanup temp input file {temp_input_path}: {cleanup_error}")
...
@celery_app.task(bind=True, name='tasks.storage_processing.batch_process_from_storage', queue='audio_features')
        ...
        for i, storage_path in enumerate(storage_paths):
            ...
            if processing_type == "features":
                result = process_audio_features_from_storage.apply_async(
                    args=[individual_task_id, storage_path, False, callback_url, batch_metadata]
                ).get()
            elif processing_type == "stems":
                result = separate_audio_stems_from_storage.apply_async(
                    args=[individual_task_id, storage_path, model_name, callback_url, batch_metadata]
                ).get()
            else:
