...
from services.storage_service import get_storage_service, get_storage_type, StorageType, StorageError
...
def create_pedalboard_for_preset(preset_config: Dict, tempo_factor: float, pitch_shift_semitones: float, add_reverb: bool = False, stem_type: str = "full_mix") -> Optional[Pedalboard]:
    """
        ...
        if preset_config and "effects" in preset_config:
            ...
            if "brightness_boost" in preset_effects:
                if stem_type == "vocals":
                    ...
                    effects.append(HighShelfFilter(cutoff_frequency_hz=4000, gain_db=1.5))
                elif stem_type == "drums":
                    # Enhance cymbal clarity
                    effects.append(HighShelfFilter(cutoff_frequency_hz=8000, gain_db=3.0))
                else:
                    # Standard brightness boost
                    effects.append(HighShelfFilter(cutoff_frequency_hz=3000, gain_db=2.0))
            ...
            if "low_pass_filter" in preset_effects:
                ...
                effects.append(LowShelfFilter(cutoff_frequency_hz=cutoff_hz, gain_db=gain_db))
            ...
            if "compression" in preset_effects:
                ...
                if stem_type == "vocals":
                    ...
                    effects.append(Compressor(threshold_db=threshold + 2, ratio=max(2.0, ratio * 0.75), attack_ms=5, release_ms=50))
                elif stem_type == "drums":
                    # Punchy drum compression
                    effects.append(Compressor(threshold_db=threshold - 3, ratio=min(8.0, ratio * 1.5), attack_ms=1, release_ms=30))
                else:
                    # Use preset compression settings
                    effects.append(Compressor(threshold_db=threshold, ratio=ratio, attack_ms=10, release_ms=100))
                logger.info(f"Added compression (threshold: {threshold}dB, ratio: {ratio}:1) for {stem_type}")
            ...
            if "chorus" in preset_effects:
                if stem_type != "drums":  # Skip chorus on drums
                    ...
                    logger.info(f"Added chorus effect (rate: {rate_hz}Hz, depth: {depth}) for {stem_type}")
        
        # Add reverb effect
        if add_reverb:
            reverb_settings = preset_config.get("reverb_settings", {}) if preset_config else {}
            ...
            logger.info(f"Added reverb (wet: {wet_level}, room: {room_size}) for {stem_type}")
...
def calculate_optimal_tempo_factor(current_bpm: float, target_style: str) -> float:
    ...
    if target_style in BPM_TARGETS:
        ...
        logger.info(f"BPM-aware suggestion: {current_bpm:.1f} -> {target_bpm:.1f} BPM (factor: {suggested_factor:.2f})")
        return suggested_factor
    
    return 1.0


def get_smart_preset_suggestions(current_bpm: float, audio_duration: float) -> Dict[str, Dict]:
    """
...
def process_stems_individually(stems_data: Dict[str, np.ndarray], sample_rate: int, preset_config: Dict, tempo_factor: float, pitch_shift_semitones: float, preserve_pitch: bool, add_reverb: bool) -> Dict[str, np.ndarray]:
    """
    ...
    for stem_name, stem_audio in stems_data.items():
        ...
        try:
            ...
            processed_stem = apply_tempo_processing(
                stem_audio, sample_rate, tempo_factor,
                pitch_shift_semitones, preserve_pitch, add_reverb,
                preset_config, stem_type=stem_name
            )
...
def mix_processed_stems(processed_stems: Dict[str, np.ndarray], mixing_weights: Optional[Dict[str, float]] = None) -> np.ndarray:
    """
    ...
    for stem_name, stem_audio in processed_stems.items():
        ...
        if len(stem_audio) < max_length:
            padded_stem = np.pad(stem_audio, (0, max_length - len(stem_audio)), 'constant')
        else:
...
def validate_tempo_processing_params(tempo_factor: float, pitch_shift_semitones: float, audio_duration: float) -> Dict[str, str]:
    """
    ...
    if pitch_shift_semitones < -12 or pitch_shift_semitones > 12:
        raise ValueError(f"Pitch shift {pitch_shift_semitones} out of range [-12, 12] semitones")
    ...
    if audio_duration > 600:  # 10 minutes
        warnings["long_duration"] = "Long audio files may take significant processing time"
    elif audio_duration < 10:  # Very short
        warnings["short_duration"] = "Very short audio may not benefit from tempo processing"
    
    # Combined effects warnings
    if tempo_factor > 1.5 and abs(pitch_shift_semitones) > 4:
        warnings["aggressive_processing"] = "Combining high tempo and pitch changes may degrade quality"
    
    return warnings


def handle_processing_error(error: Exception, context: str, audio_data: Optional[np.ndarray] = None) -> Dict[str, Any]:
    """
    ...
    if isinstance(error, ValueError):
        if "tempo_factor" in str(error).lower():
            error_info["recovery_suggestions"].append("Adjust tempo_factor to range [0.25, 4.0]")
        elif "pitch_shift" in str(error).lower():
            error_info["recovery_suggestions"].append("Adjust pitch_shift_semitones to range [-12, 12]")
        elif "audio" in str(error).lower():
            error_info["recovery_suggestions"].append("Check audio file format and content")
    ...
    if audio_data is not None:
        error_info["audio_context"] = {
            ...
            "length_seconds": len(audio_data) / 44100 if hasattr(audio_data, '__len__') else None
        }
...
def check_system_compatibility() -> Dict[str, Any]:
    ...
    if not PEDALBOARD_AVAILABLE:
        compatibility["recommendations"].append("Install pedalboard for enhanced audio effects")
    
    # Check memory availability (basic check)
    try:
        import psutil
        memory_info = psutil.virtual_memory()
        compatibility["system_info"]["available_memory_gb"] = memory_info.available / (1024**3)
        
        if memory_info.available < 2 * (1024**3):  # Less than 2GB
            compatibility["recommendations"].append("Low memory detected - consider smaller audio files")
    except ImportError:
    ...
    try:
        ...
        if platform.machine() == 'arm64' and platform.system() == 'Darwin':
            ...
            compatibility["recommendations"].append("Apple Silicon detected - CPU processing will be used for compatibility")
    except Exception:
        pass
...
def apply_tempo_processing(audio_data, sample_rate, tempo_factor, pitch_shift_semitones, preserve_pitch, add_reverb, preset_config=None, stem_type="full_mix"):
    ...
    try:
        ...
        processing_params = {
            ...
            "preset": preset_config.get("preset", "custom") if preset_config else "custom",
            "stem_type": stem_type
        }
        ...
        if cached_result:
            ...
            monitor.record_processing_time(processing_time, processing_params["preset"], 
                                         len(audio_data) * 4 / (1024 * 1024), cache_hit=True)
            logger.info(f"🚀 Cache hit for tempo processing: {cache_key[:8]}... (saved {processing_time:.3f}s)")
        ...
        if tempo_factor != 1.0:
            if preserve_pitch:
                ...
                processed_audio = librosa.effects.time_stretch(processed_audio, rate=tempo_factor)
                logger.info(f"Applied time stretch with factor {tempo_factor} (pitch preserved) for {stem_type}")
            else:
                # For non-pitch preserving, we'll combine tempo + pitch changes
                processed_audio = librosa.effects.time_stretch(processed_audio, rate=tempo_factor)
                logger.info(f"Applied time stretch with factor {tempo_factor} for {stem_type}")
        
        # Step 2: Apply pedalboard effects for enhanced audio processing
        if PEDALBOARD_AVAILABLE:
            logger.debug(f"Using pedalboard for audio effects processing on {stem_type}")
            ...
            pedalboard_fx = create_pedalboard_for_preset(
                preset_config, tempo_factor, pitch_shift_semitones, add_reverb, stem_type
            )
            ...
            logger.warning(f"Pedalboard not available - using fallback audio processing for {stem_type}")
            ...
            if pitch_shift_semitones != 0.0:
                ...
                logger.info(f"Applied pitch shift of {pitch_shift_semitones} semitones to {stem_type}")
        ...
        if processing_time > 0.5:  # Only cache if processing took more than 0.5 seconds
            cache.cache_processed_audio(cache_key, result, sample_rate, processing_params)
            logger.debug(f"💾 Cached tempo processing result (took {processing_time:.2f}s): {cache_key[:8]}...")
        
        # Record performance metrics
        file_size_mb = len(audio_data) * 4 / (1024 * 1024)  # Estimate for float32 array
        monitor.record_processing_time(processing_time, processing_params["preset"], file_size_mb, cache_hit=False)
...
def process_tempo_from_storage(
    ...
    try:
        current_task.update_state(
            state='PROGRESS',
            meta={'progress': 0, 'status': 'Initializing tempo processing', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS',
            meta={'progress': 10, 'status': 'Downloading file from storage', 'task_id': task_id}
        )
        ...
        current_task.update_state(
            state='PROGRESS',
            meta={'progress': 70, 'status': 'Saving processed audio', 'task_id': task_id}
        )
        ...
        if not storage.upload_file(temp_output_path, output_path):
            raise StorageError(f"Failed to upload processed audio to storage: {output_path}")
        ...
        error_info = handle_processing_error(e, f"tempo processing task {task_id}", audio_data if 'audio_data' in locals() else None)
        ...
        if callback_url:
            try:
                ...
                logger.error(f"Failed to send failure callback for tempo task {task_id}: {callback_error}")
...
@celery_app.task(bind=True, name='tasks.tempo_processing.process_tempo_direct', queue='tempo_processing')
def process_tempo_direct(
    self,
    task_id: str,
    audio_data: bytes,
    filename: str,
    ...
    """
    Process tempo modifications for directly uploaded files (Phase 2 - enhanced implementation)
    Saves processed audio to temporary file and returns file location
    """
    start_time = time.time()
    temp_output_path = None
    ...
    try:
        current_task.update_state(
            state='PROGRESS',
            meta={'progress': 0, 'status': 'Processing uploaded file', 'task_id': task_id}
        )
        ...
        logger.info(f"Loaded uploaded audio: {audio.shape} samples at {sample_rate} Hz")
        ...
        # Extract original BPM for analysis
        original_bpm = librosa.beat.tempo(y=audio, sr=sample_rate)[0]
        ...
        # Apply tempo processing with preset configuration
        processed_audio = apply_tempo_processing(
            audio, sample_rate, tempo_factor,
            pitch_shift_semitones, preserve_pitch, add_reverb,
            preset_config
        )
        ...
        current_task.update_state(
            state='PROGRESS',
            meta={'progress': 70, 'status': 'Saving processed audio to temporary file', 'task_id': task_id}
        )
        
        # Save processed audio to temporary file with descriptive filename
        base_name = Path(filename).stem if filename else "processed_audio"
        preset_suffix = preset if preset != "custom" else f"tempo_{tempo_factor:.2f}"
        output_filename = f"{base_name}_tempo_{preset_suffix}_{task_id[:8]}.wav"
        
        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False, prefix=f"{base_name}_tempo_") as temp_file:
            temp_output_path = temp_file.name
        
        # Save the processed audio
        sf.write(temp_output_path, processed_audio, sample_rate)
        
        # Create a more user-friendly filename in the same directory
        temp_dir = os.path.dirname(temp_output_path)
        user_friendly_path = os.path.join(temp_dir, output_filename)
        
        # Move/rename the file to the user-friendly name
        os.rename(temp_output_path, user_friendly_path)
        temp_output_path = user_friendly_path
        
        # Log the file location prominently
        logger.info(f"🎵 PROCESSED AUDIO SAVED: {temp_output_path}")
        logger.info(f"📁 File accessible at: {temp_output_path}")
        ...
        current_task.update_state(
            state='PROGRESS',
            meta={'progress': 100, 'status': 'Completed', 'task_id': task_id, 'output_file': temp_output_path}
        )
        
        # Final prominent log entry for easy access
        logger.info(f"✅ TASK {task_id} COMPLETE - Processed audio available at: {temp_output_path}")
        ...
        # Enhanced error handling with detailed context
        error_info = handle_processing_error(e, f"direct tempo processing task {task_id}", audio if 'audio' in locals() else None)
        
        logger.error(f"Error in direct tempo processing task {task_id}: {error_msg}")
        ...
        if temp_output_path and os.path.exists(temp_output_path):
            try:
                ...
                logger.warning(f"Failed to cleanup temp file {temp_output_path}: {cleanup_error}")
