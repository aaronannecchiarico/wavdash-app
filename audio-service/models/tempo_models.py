...
class StorageTempoProcessingRequest(BaseModel):
    ...
    callback_url: Optional[str] = Field(None, description="URL to notify when processing is complete")
...
class TempoProcessingResponse(BaseModel):
    ...
    original_analysis: Optional[Dict[str, Any]] = Field(None, description="Original audio analysis data")
    tempo_processing: Optional[Dict[str, Any]] = Field(None, description="Tempo processing details")
    output_files: Optional[Dict[str, Any]] = Field(None, description="Output file paths")
    output_file: Optional[Dict[str, Any]] = Field(None, description="Direct processing output file info (path, filename, size, etc.)")
    public_urls: Optional[Dict[str, str]] = Field(None, description="Public URLs for processed files")
    processing_time: Optional[float] = Field(None, description="Processing time in seconds")
    storage_type: Optional[str] = Field(None, description="Type of storage used (local/r2)")
    smart_suggestions: Optional[Dict[str, Any]] = Field(None, description="AI-generated preset suggestions based on audio characteristics")
    processing_warnings: Optional[Dict[str, str]] = Field(None, description="Validation warnings about processing parameters")
...
class TempoPresetConfig(BaseModel):
    ...
    tempo_factor: float = Field(..., ge=0.25, le=4.0, description="Tempo multiplication factor")
    pitch_shift_semitones: float = Field(..., ge=-12.0, le=12.0, description="Pitch shift in semitones")
    ...
    reverb_settings: Optional[Dict[str, float]] = Field(None, description="Reverb effect settings")
    filter_settings: Optional[Dict[str, float]] = Field(None, description="Filter effect settings")
...
class TempoCallbackData(BaseModel):
    ...
    original_analysis: Optional[Dict[str, Any]] = Field(None, description="Original audio analysis")
    tempo_processing: Optional[Dict[str, Any]] = Field(None, description="Tempo processing details")
    ...
    def validate_status(cls, v):
        if v not in ['completed', 'failed', 'pending', 'processing']:
            raise ValueError('status must be one of: completed, failed, pending, processing')
        return v
