...
router = APIRouter(prefix="/migration", tags=["Migration & Maintenance"])
...
def _run_migration_script(
    script_name: str, force: bool = True, no_tests: bool = True
) -> tuple[bool, str, str]:
...
async def run_fresh_migration():
    ...
    migration_details = {"steps_completed": [], "steps_failed": [], "warnings": []}
    ...
    try:
        ...
        if not success:
            ...
            migration_details["warnings"].append(
                "Migration completed but seeding failed"
            )
        else:
