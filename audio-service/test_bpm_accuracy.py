...
def calculate_accuracy_metrics(predicted: float, actual: float) -> Dict[str, float]:
    ...
    if actual == 0:
        return {'error': 0.0, 'percent_error': 0.0, 'within_5_percent': False, 'within_10_percent': False}
    
    error = abs(predicted - actual)
    percent_error = (error / actual) * 100
    
    # Check for tempo doubling/halving (common errors)
    doubling_candidates = [predicted, predicted * 2, predicted / 2, predicted * 4, predicted / 4]
    best_candidate_error = min(abs(candidate - actual) for candidate in doubling_candidates if candidate > 0)
    best_candidate_percent = (best_candidate_error / actual) * 100 if actual > 0 else 100
...
def test_single_file(extractor, audio_file: Path, expected_bpm: float) -> Dict[str, any]:
    ...
    try:
        ...
        standard_bpm = result_standard.get('features', {}).get('tempo', {}).get('bpm', 0)
        ...
        print(f"    Standard: {standard_bpm:.1f} BPM (error: {standard_metrics['percent_error']:.1f}%)")
        print(f"    Enhanced: {enhanced_bpm:.1f} BPM (error: {enhanced_metrics['percent_error']:.1f}%, confidence: {enhanced_result.get('confidence', 0):.2f})")
...
def main():
    ...
    standard_within_10 = sum(1 for r in valid_results if r['standard_accuracy']['within_10_percent'])
    standard_within_5 = sum(1 for r in valid_results if r['standard_accuracy']['within_5_percent'])
    
    # Enhanced method statistics
    enhanced_errors = [r['enhanced_accuracy']['percent_error'] for r in valid_results]
    enhanced_within_10 = sum(1 for r in valid_results if r['enhanced_accuracy']['within_10_percent'])
    enhanced_within_5 = sum(1 for r in valid_results if r['enhanced_accuracy']['within_5_percent'])
    
    # Enhanced method with tempo correction
    enhanced_corrected_within_10 = sum(1 for r in valid_results if r['enhanced_accuracy']['within_10_percent_corrected'])
    enhanced_corrected_within_5 = sum(1 for r in valid_results if r['enhanced_accuracy']['within_5_percent_corrected'])
    
    print("\n🎯 Standard BPM Detection:")
    print(f"  Mean error: {np.mean(standard_errors):.1f}%")
    print(f"  Median error: {np.median(standard_errors):.1f}%")
    print(f"  Within 5%: {standard_within_5}/{len(valid_results)} ({100*standard_within_5/len(valid_results):.1f}%)")
    print(f"  Within 10%: {standard_within_10}/{len(valid_results)} ({100*standard_within_10/len(valid_results):.1f}%)")
    
    print("\n⚡ Enhanced BPM Detection:")
    print(f"  Mean error: {np.mean(enhanced_errors):.1f}%")
    print(f"  Median error: {np.median(enhanced_errors):.1f}%")
    print(f"  Within 5%: {enhanced_within_5}/{len(valid_results)} ({100*enhanced_within_5/len(valid_results):.1f}%)")
    print(f"  Within 10%: {enhanced_within_10}/{len(valid_results)} ({100*enhanced_within_10/len(valid_results):.1f}%)")
    
    print("\n🔧 Enhanced with Tempo Correction:")
    print(f"  Within 5%: {enhanced_corrected_within_5}/{len(valid_results)} ({100*enhanced_corrected_within_5/len(valid_results):.1f}%)")
    print(f"  Within 10%: {enhanced_corrected_within_10}/{len(valid_results)} ({100*enhanced_corrected_within_10/len(valid_results):.1f}%)")
    ...
    sorted_results = sorted(valid_results, key=lambda x: x['enhanced_accuracy']['percent_error'], reverse=True)
    for result in sorted_results[:5]:
        print(f"  {result['file']}: {result['enhanced_accuracy']['percent_error']:.1f}% error "
              f"(expected {result['expected_bpm']}, got {result['enhanced_bpm']:.1f})")
    
    # Show best performing files
    print(f"\n✅ Most accurate results:")
    for result in sorted(valid_results, key=lambda x: x['enhanced_accuracy']['percent_error'])[:5]:
        print(f"  {result['file']}: {result['enhanced_accuracy']['percent_error']:.1f}% error "
              f"(expected {result['expected_bpm']}, got {result['enhanced_bpm']:.1f})")
    
    # Method analysis
    print(f"\n🔍 Method Performance Analysis:")
    method_stats = {'standard': [], 'percussive': [], 'onset_based': [], 'tempogram': []}
    ...
    for method_name, errors in method_stats.items():
        if errors:
            print(f"  {method_name.capitalize()}: {np.mean(errors):.1f}% mean error ({len(errors)} valid results)")
        else:
    ...
    try:
        ...
        for result in valid_results:
            df_data.append({
                ...
                'standard_error_percent': result['standard_accuracy']['percent_error'],
                'enhanced_error_percent': result['enhanced_accuracy']['percent_error'],
                'enhanced_corrected_error_percent': result['enhanced_accuracy']['best_candidate_percent']
            })
