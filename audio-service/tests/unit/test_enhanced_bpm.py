...
from services.audio_feature_extraction import AudioFeatureExtractor, FeatureExtractionConfig
...
class TestEnhancedBPM:
    ...
    def create_test_audio(self, bpm: float, duration: float = 4.0) -> np.ndarray:
        ...
        click = 0.5 * np.exp(-np.linspace(0, 5, click_samples)) * np.sin(2 * np.pi * 1000 * np.linspace(0, click_duration, click_samples))
    ...
    def test_tempo_confidence_calculation(self):
        ...
        confidence = self.extractor._calculate_tempo_confidence(high_agreement_tempos, final_tempo)
        assert confidence > 0.8
        
        # Low agreement case
        low_agreement_tempos = [80, 120, 160, 200]
        confidence = self.extractor._calculate_tempo_confidence(low_agreement_tempos, final_tempo)
    ...
    def test_enhanced_bpm_with_known_values(self):
        ...
        for target_bpm in test_bpms:
            ...
            assert abs(result['bpm'] - target_bpm) <= tolerance, \
    ...
    def test_enhanced_bpm_error_handling(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", UserWarning)  # Suppress librosa warnings for short audio
    ...
    def test_method_results_validity(self):
        ...
        for method_name, tempo in methods.items():
            ...
            if tempo > 0:
                assert 30 <= tempo <= 300, f"{method_name} returned unreasonable tempo: {tempo}"
    ...
    def test_enhanced_vs_standard_comparison(self):
        ...
        enhanced_result = self.extractor.extract_bpm_enhanced(test_audio, self.sample_rate)
