...
class TestTempoProcessingFunctions:
    ...
    def test_apply_tempo_processing_slow_down(self, mock_time_stretch):
        ...
        tempo_factor = 0.75
        expected_length = int(len(self.test_audio) / tempo_factor)
        mock_output = np.tile(self.test_audio, int(np.ceil(expected_length / len(self.test_audio))))[:expected_length]
        mock_time_stretch.return_value = mock_output
    ...
    @patch('tasks.tempo_processing.PEDALBOARD_AVAILABLE', False)  # Force fallback to librosa
    @patch('librosa.effects.pitch_shift')
    @patch('librosa.effects.time_stretch')
    def test_apply_tempo_processing_pitch_shift_with_time_stretch(self, mock_time_stretch, mock_pitch_shift):
    ...
    @patch('librosa.effects.time_stretch')
    def test_apply_tempo_processing_with_pedalboard_available(self, mock_time_stretch, mock_create_pedalboard):
...
class TestTempoProcessingEdgeCases:
    ...
    def test_extreme_tempo_factors(self, mock_time_stretch):
        ...
        test_audio = np.sin(2 * np.pi * 440 * np.linspace(0, duration, samples)).astype(np.float32)
    ...
    def test_empty_audio_array(self):
        ...
        with pytest.raises(ValueError, match="Audio data cannot be None or empty"):
            apply_tempo_processing(
                empty_audio, 22050, 1.0, 0.0, False, False
            )
