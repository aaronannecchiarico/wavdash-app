...
class TestTempoModels:
    ...
    def test_tempo_preset_config_invalid_ranges(self):
        ...
        with pytest.raises(ValidationError):
            TempoPresetConfig(
                name="Invalid", tempo_factor=0.1, pitch_shift_semitones=0.0  # Too low
            )

        with pytest.raises(ValidationError):
            TempoPresetConfig(
                name="Invalid", tempo_factor=1.0, pitch_shift_semitones=15.0  # Too high
            )
    ...
    def test_tempo_callback_data_valid(self):
        ...
        callback_data = TempoCallbackData(
            ...
            storage_paths={
                "original": "uploads/test.wav",
                "processed": "processed/test_tempo.wav",
            },
