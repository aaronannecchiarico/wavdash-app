...
class TestTempoCallbackData:
    ...
    def test_callback_data_with_warnings(self):
        ...
        assert "Very fast tempo may cause audio artifacts" in tempo["processing_warnings"]
        assert "Large pitch shifts may cause unnatural sound" in tempo["processing_warnings"]
        assert "Combining high tempo and pitch changes may degrade quality" in tempo["processing_warnings"]
    ...
    def test_failed_callback_data(self):
        ...
        callback_data = TempoCallbackData(
            ...
            processing_type="tempo",
            storage_paths={
                "original": "uploads/789/2025/08/15/corrupted.mp3"
            },
...
class TestCallbackDataIntegration:
    ...
    def test_callback_sent_with_complete_data(self, mock_post):
        ...
        with patch('tasks.tempo_processing.get_storage_service') as mock_storage_service, \
            ...
            mock_storage.get_public_url.return_value = "http://example.com/processed.wav"
            ...
            # Mock audio loading
            import numpy as np
            mock_audio_data = np.array([1, 2, 3, 4, 5], dtype=np.float32)  # Simple mock data
            ...
            if len(call_args[0]) > 0:
                assert call_args[0][0] == "http://example.com/callback"  # Positional arg
            else:
                assert call_args[1]['url'] == "http://example.com/callback"  # Keyword arg
