...
class StorageProcessingResponse(BaseModel):
    ...
    storage_analysis_path: Optional[str] = Field(None, description="Path to analysis results in storage")
    storage_processed_path: Optional[str] = Field(None, description="Path to processed audio in storage")
...
class StorageStemSeparationResponse(BaseModel):
    ...
    storage_stems_paths: Optional[Dict[str, str]] = Field(None, description="Storage paths for separated stems")
    public_urls: Optional[Dict[str, str]] = Field(None, description="Public URLs for stem files")
    processing_time: Optional[float] = Field(None, description="Processing time in seconds")
    storage_type: Optional[str] = Field(None, description="Type of storage used (local/r2)")
...
class StorageCallbackData(BaseModel):
    ...
    analysis_summary: Optional[Dict[str, Any]] = Field(None, description="Summary of analysis results")
...
class StorageBatchProcessingRequest(BaseModel):
    ...
    storage_paths: List[str] = Field(..., min_length=1, description="List of storage paths to process")
    processing_type: str = Field(..., description="Type of processing (features|stems)")
    ...
    batch_metadata: Optional[Dict[str, str]] = Field(default={}, description="Batch-level metadata")
