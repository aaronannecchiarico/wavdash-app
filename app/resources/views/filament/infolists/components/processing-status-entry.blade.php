@php
    $record = $getRecord();
    $status = $getState() ?? 'unknown';
    $progress = $record?->progress ?? 0;
    $errorMessage = $record?->error_message;
    $submittedAt = $record?->submitted_at;
    $completedAt = $record?->completed_at;
    $estimatedTimeRemaining = $entry->getCurrentTimeRemaining();
    
    $statusColor = $entry->getCurrentStatusColor();
    $statusIcon = $entry->getCurrentStatusIcon();
@endphp
...
<x-dynamic-component
    ...
    :entry="$entry"
>
    <div {{ $getExtraAttributeBag() }} class="filament-infolists-processing-status-entry">
