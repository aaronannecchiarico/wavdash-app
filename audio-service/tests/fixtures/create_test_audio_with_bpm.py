...
def create_metronome_click(sample_rate: int = 22050, frequency: float = 1000, duration: float = 0.05) -> np.ndarray:
...
def create_musical_phrase(bpm: float, sample_rate: int = 22050, duration: float = 8.0) -> np.ndarray:
    ...
    for beat in range(beats_in_duration):
        ...
        if bpm > 100:
            ...
            if offbeat_sample + len(hihat) <= len(audio):
                ...
                audio[offbeat_sample:end_sample] += hihat[:end_sample - offbeat_sample] * 0.6
    ...
    for beat in range(beats_in_duration // 4):  # Change chord every 4 beats
        ...
        bass_line = 0.3 * np.sin(2 * np.pi * freq * t) * np.exp(-((t - start_time) % beat_duration) * 8)
...
def create_complex_rhythm(bpm: float, sample_rate: int = 22050, duration: float = 8.0) -> np.ndarray:
    ...
    for pattern in range(num_patterns):
        ...
        for sixteenth in range(32):  # 8 beats * 4 sixteenths
            ...
            sample_pos = int((pattern_start + sixteenth * sixteenth_duration) * sample_rate)
            if sample_pos + len(hihat) <= len(audio):
                end_sample = min(sample_pos + len(hihat), len(audio))
                audio[sample_pos:end_sample] += hihat[:end_sample - sample_pos] * volume
    
    # Add melodic content to make it more musical
    t = np.linspace(0, duration, total_samples)
    melody_freq = 440  # A4
    melody = 0.2 * np.sin(2 * np.pi * melody_freq * t) * (np.sin(2 * np.pi * t / beat_duration) ** 2)
    audio += melody
...
def main():
    ...
    triplet_audio = create_musical_phrase(90, sample_rate, duration)  # 90 BPM with triplet subdivision
    ...
    with open(reference_file, 'w') as f:
        ...
        for bpm, description in test_bpms:
            f.write(f"{description}_{bpm}bpm_simple.wav,{bpm},Simple rhythmic pattern\n")
            f.write(f"{description}_{bpm}bpm_complex.wav,{bpm},Complex rhythmic pattern\n")
