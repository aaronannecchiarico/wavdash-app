...
class PerformanceCache:
    ...
    def __init__(self, cache_dir: Optional[str] = None, max_cache_size_mb: int = 500):
        self.cache_dir = Path(cache_dir) if cache_dir else Path(tempfile.gettempdir()) / "beatforge_cache"
        ...
        try:
            ...
            if total_size_mb > self.max_cache_size_mb:
                ...
                for cache_file in cache_files:
                    ...
                    # Recalculate size
                    remaining_size = sum(f.stat().st_size for f in self.cache_dir.rglob("*.cache"))
                    if remaining_size / (1024 * 1024) <= self.max_cache_size_mb * 0.8:  # 80% of limit
                        break
    ...
    def cache_bpm_analysis(self, audio_hash: str, bpm_data: Dict[str, Any], max_age_hours: int = 168):
    ...
    def get_cached_bpm_analysis(self, audio_hash: str, max_age_hours: int = 168) -> Optional[Dict[str, Any]]:
    ...
    def cache_processed_audio(self, cache_key: str, audio_data: np.ndarray, sample_rate: int, 
                            processing_params: Dict, max_age_hours: int = 48):
    ...
    def get_cached_processed_audio(self, cache_key: str, max_age_hours: int = 48) -> Optional[Dict[str, Any]]:
    ...
    def generate_audio_cache_key(self, audio_hash: str, processing_params: Dict) -> str:
        ...
        key_data = {
            ...
            "pitch_shift_semitones": processing_params.get("pitch_shift_semitones", 0.0),
...
def audio_hash(audio_data: Union[np.ndarray, bytes]) -> str:
    ...
    return hashlib.md5(hash_input if isinstance(hash_input, bytes) else hash_input.encode()).hexdigest()
...
def cached_tempo_processing(cache_ttl_hours: int = 48):
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            processing_params = {
                "tempo_factor": kwargs.get('tempo_factor', args[2] if len(args) > 2 else 1.0),
                "pitch_shift_semitones": kwargs.get('pitch_shift_semitones', args[3] if len(args) > 3 else 0.0),
                "preserve_pitch": kwargs.get('preserve_pitch', args[4] if len(args) > 4 else False),
                "add_reverb": kwargs.get('add_reverb', args[5] if len(args) > 5 else False),
            ...
            if processing_time > 1.0 and isinstance(result, np.ndarray):
                sample_rate = args[1] if len(args) > 1 else kwargs.get('sample_rate', 44100)
                cache.cache_processed_audio(cache_key, result, sample_rate, processing_params, cache_ttl_hours)
                logger.info(f"Cached tempo processing result (took {processing_time:.1f}s): {cache_key[:8]}...")
...
class PerformanceMonitor:
    ...
    def record_processing_time(self, processing_time: float, preset: str = "custom", 
                             file_size_mb: float = 0, cache_hit: bool = False):
        ...
        # Track preset usage
        self.metrics["preset_usage"][preset] = self.metrics["preset_usage"].get(preset, 0) + 1
        
        # Update average file size
        if file_size_mb > 0:
            current_avg = self.metrics["average_file_size_mb"]
            total_requests = self.metrics["total_requests"]
            self.metrics["average_file_size_mb"] = ((current_avg * (total_requests - 1)) + file_size_mb) / total_requests
